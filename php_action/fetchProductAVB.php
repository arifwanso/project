...
$sql = "SELECT product.product_id, product.product_name, product.product_image,
 		...
 		type.type_name,product.warning FROM product,type where product.type_id = type.type_id AND product.warning < product.quantity "; 
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[7] == 1 ){
 		// activate member
 		$status = "<label class='label label-success'>มีสินค้า</label>";
 	} elseif($row[7] == 2) {
 		// deactivate member
